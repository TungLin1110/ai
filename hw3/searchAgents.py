...
class CornersProblem(search.SearchProblem):
  ...
  def __init__(self, startingGameState):
    ...
    visited_corner = [0, 0, 0, 0]
    self.startState=(self.startingPosition, tuple(visited_corner))
    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      if not self.walls[nextx][nexty]:
          ...
          if nextState in self.corners:
             ...
             visited_corner = list(deepcopy(state[1]))
             visited_corner[index] = 1
          else:
             visited_corner = list(deepcopy(state[1]))
                 
          successors.append(((nextState, tuple(visited_corner)), action, cost))
    self._expanded += 1
    if state not in self._visited:
        self._visited[state] = True
...
def cornersHeuristic(state, problem):
  ...
  while len(unvisited_corners) != 0:
    dis , cor = min ([(euclideanHeuristic(now_coordinate,cor),cor)for cor in unvisited_corners])
    cost = cost + dis
    now_coordinate = cor
